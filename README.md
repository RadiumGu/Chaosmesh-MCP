...
**Pros:** Portable, no AWS dependency at runtime, least-privilege permissions (Chaos Mesh only)
**Cons:** Token has a fixed expiry (default 1 year), needs renewal
...
**Pros:** Uses existing admin credentials, no extra setup needed
**Cons:** Depends on AWS CLI + IAM credentials, admin-level permissions (overly broad)
